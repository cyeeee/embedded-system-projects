...
/**
 * Receives character from the serial terminal by reading
 * a byte of data from the UART receive data Register.
 *
 * @param uart_p: The pointer to the UART instance with which to handle our operations.
 *
...
{
    char rxChar = UART_receiveData(uart_p->moduleInstance);
    return rxChar;
}
...
//       implemented over simply WHAT the function does.
/**
 * Check if the UART module can send the character by checking
 * the interrupt status of the proper system UART module.
 *
 * @param uart_p: The pointer to the UART instance with which to handle our operations.
 *
 * @return true if the UART module can send the character, and false otherwise
 */
bool UART_canSend(UART* uart_p)
{
    uint8_t uartFlags = UART_getInterruptStatus(
            uart_p->moduleInstance, EUSCI_A_UART_TRANSMIT_INTERRUPT);
    return (uartFlags == EUSCI_A_UART_TRANSMIT_INTERRUPT);
}
...
//       implemented over simply WHAT the function does.
/**
 * Send the character by placing the supplied data into
 * UART transmit data register to start transmission
 *
 * @param uart_p: The pointer to the UART instance with which to handle our operations.
 */
