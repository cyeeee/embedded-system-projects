...
{
    gfx_p->border.xMin = x_min;
    gfx_p->border.yMin = y_min;
    gfx_p->border.xMax = x_max;
    gfx_p->border.yMax = y_max;
    Graphics_drawRectangle(&gfx_p->context, &gfx_p->border);
}
...
{
    uint32_t oldForegroundColor = gfx_p->foreground;
    GFX_setForeground(gfx_p, HIGHLIGHT);
    GFX_drawRectangle(gfx_p, x_min, y_min, x_max, y_max);
    GFX_setForeground(gfx_p, oldForegroundColor);
}
